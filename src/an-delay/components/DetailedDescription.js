...
function DetailedDescription({ path }) {
  return (
    <div
      ...
      `}
    >
      {path ? path.map((step, index) => <DescriptionStep key={index} step={step} />) : ''}
    </div>
  )
}
...
function DescriptionStep({ step }) {
  ...
  if (step.annotatedDescription) {
    description.push(
      step.annotatedDescription.map(({ type, value }, index) => {
        ...
        if (type === 'address' || type === 'any-account') {
          return (
            <span key={key}>
              <LocalIdentityBadge compact entity={type === 'any-account' ? 'Any account' : value} />
            </span>
          )
        }
    ...
    description.push(
      <span key={description.length + 1}>{step.description || 'No description'}</span>
    )
  }
